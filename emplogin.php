...
<br>
	...
	<h2 class="w3-center">EMPLOYEE Login</h2>
		...
		<?php
			if(isset($_POST['login']))
			{
				...
				$query = "select * from empdetail where (username='$username' OR emp_no='$username') and password='$password' ";
				//echo $query;
				...
				if($query_run)
				{
					...
					if($row['password']=='rcf@123'){
						header("location:empinfoupdate.php");
						$_SESSION['emp_no'] = $row["emp_no"];
					}else{
					$_SESSION['empusername'] = $row['username'];
					$_SESSION['emp_no'] = $row["emp_no"];
