...
<script>
		function ColGenerator()
        {
            $("#autoCOL").show();
            var i;
            var rNo = parseInt($("#NoOfEmp").val());
            for (i = 0; i < rNo; i++) {
              $('#autoCOL')
               .append('<tr><td><input id="empno' + i + '" type="text" class="input" /></td>' +
                   '</tr>');
            }
			$("#NoOfEmpTable").hide();
            ...
            $("#ADD").click(function(){
                  ...
                  while (i!=rNo) {
                    if(i==rNo-1)
						values += "('" + $('#empno' + i).val() + "','rcf@123');" 
					else
						values += "('" + $('#empno' + i).val() + "','rcf@123'),"
					i++;
                  }
                  var query = "INSERT INTO empdetail(emp_no,password) values " +values;
                  var tb_tag = "addemp";
...
<div class="w3-sidebar w3-bar-block w3-collapse w3-card-2 w3-animate-left" style="width:200px;" id="mySidebar">
		...
		<a class="w3-bar-item w3-button w3-green" href="addemp.php">Add New Employee</a>
...
<p style="font-size: 9pt;">Note: Default password of newly added employee(s) is 'rcf@123' (without quotes)</p>
	<table id="NoOfEmpTable">
		<tr>
		<td><label class="w3-label w3-text-blue">Number of Employees to add<span class="w3-text-red">*</span>:&nbsp</label></td>
		<td><input id="NoOfEmp" class="w3-input w3-border" type="text" style="width:90%"></td>
	...
	<p class="w3-right"><button id="ADD" class="w3-btn w3-red w3-round-xlarge">Add Employee(s)</button></p>
</div>
