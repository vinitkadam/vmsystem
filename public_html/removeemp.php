...
<script>
	...
	{
		var empno = $("#empno").val();
        var query = " DELETE FROM empdetail WHERE emp_no='"+empno+"';";
        var tb_tag = "removeemp";
	...
	function remove(emp_no)
	{
		var query = " DELETE FROM empdetail WHERE emp_no='"+emp_no+"';";
        var tb_tag = "removeemp";
        var dataValues = 'tag='+ tb_tag + '&Query='+ query;
        $.ajax({
        type: "POST",
...
<div class="w3-sidebar w3-bar-block w3-collapse w3-card-2 w3-animate-left" style="width:200px;" id="mySidebar">
		...
		<a class="w3-bar-item w3-button w3-green" href="removeemp.php">Remove Employee</a>
...
<div class="w3-container ">
		...
		<table id="empnotable">
		<tr>
		<td><label class="w3-label w3-text-blue">Employee Id:<span class="w3-text-red">*</span>:&nbsp</label></td>
		<td><input id="empno" class="w3-input w3-border" type="text" style="width:90%"></td>
		<td><button class="w3-btn w3-red w3-round-large" onclick="removeemp();">REMOVE</button></td>
		</tr>
		</table>
	...
	<?php
		...
		$query="SELECT * from empdetail";
		if($result = mysqli_query($con,$query))
		{
			
			if(mysqli_num_rows($result) > 0)
			{
				echo "<tr> <th></th><th>username</th>  <th>Name</th> <th>Email</th> <th>Employee No.</th> <th>Mobile No.</th> </tr>";
				while($row = mysqli_fetch_array($result))
				{
					...
					$mob_no=$row["mob_no"];
					echo "<tr id='$emp_no'>
							<td><button class='w3-btn w3-blue-grey w3-small w3-round-xxlarge' onclick='remove($emp_no);'>&times; remove</button></td>
							<td>".$username."</td>
							<td>".$name."</td>
							<td>".$email."</td>
							<td>".$emp_no."</td>
					...
					echo "No Employee details to display";
		}
		else
		{
