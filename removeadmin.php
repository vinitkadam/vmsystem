...
<script>
	function removeadmin()
	{
        ...
        var query = " DELETE FROM admin WHERE admin_no='"+adminno+"';";
        var tb_tag = "removeadmin";
        var dataValues = 'tag='+ tb_tag + '&Query='+ query+'&adminno='+adminno;
        $.ajax({
        type: "POST",
        ...
        success: function(result){
			...
			document.getElementById(admin_no).style.display="none";			
			}
        });
    }
	
	function remove(admin_no)
	{
		var query = "DELETE FROM admin WHERE admin_no='"+admin_no+"';";
        var tb_tag = "removeadmin";
        var dataValues = 'tag='+ tb_tag + '&Query='+ query+'&adminno='+admin_no;
        $.ajax({
        type: "POST",
        url: "adminexecutor.php",
        data: dataValues,
...
<div class="w3-sidebar w3-bar-block w3-collapse w3-card-2 w3-animate-left" style="width:200px;" id="mySidebar">
		...
		<a class="w3-bar-item w3-button w3-green" href="removeadmin.php">Remove Admin</a>
...
<div class="w3-container ">
		...
		<table id="adminnotable">
		<tr>
		<td><label class="w3-label w3-text-blue">Admin No:<span class="w3-text-red">*</span>:&nbsp</label></td>
		<td><input id="adminno" class="w3-input w3-border" type="text" style="width:90%"></td>
		<td><button class="w3-btn w3-red w3-round-large" onclick="removeadmin();">REMOVE</button></td>
		</tr>
		</table>
	...
	<?php
		...
		if($result = mysqli_query($con,$query))
		{
			...
			{
				echo "<tr> <th></th><th>username</th>  <th>Name</th> <th>Email</th> <th>Admin No.</th> <th>Mobile No.</th> </tr>";
				while($row = mysqli_fetch_array($result))
				{
					...
					$mob_no=$row["mob_no"];
					echo "<tr id='$admin_no'>
							<td><button class='w3-btn w3-blue-grey w3-small w3-round-xxlarge' onclick='remove($admin_no);'>&times; remove</button></td>
							<td>".$username."</td>
							<td>".$name."</td>
							<td>".$email."</td>
							<td>".$admin_no."</td>
					...
					echo "No Admin details to display";
		}
		else
		{
